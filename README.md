...
## Скачать для macOS и Windows

Готовые архивы собираются через GitHub Actions:

- `VoiceReader-macOS.zip` - macOS `.app` сборка.
- `VoiceReader-Windows.zip` - Windows `win-x64` сборка.

Как получить сборку:

1. Откройте вкладку `Actions` в GitHub.
2. Запустите workflow `Build Downloadable Apps` вручную или создайте тег версии вида `v1.0.0`.
3. Скачайте артефакт из завершенного workflow run.

Для публичного релиза рекомендуется прикрепить эти архивы к GitHub Release.
